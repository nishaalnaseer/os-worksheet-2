...
void kernel_main() {
  ...
  for(int i = 0; i <= 31; i++) {
    if (terminal_row <= VGA_HEIGHT) {
      ...
      // moving text one row up
      for (size_t a = 0; a < VGA_HEIGHT; a++) {
        for(size_t b = 0; b < VGA_WIDTH; b++) {
          size_t index_line = a * VGA_WIDTH + b;
          size_t next_line = (a+1) * VGA_WIDTH + b;
          terminal_buffer[index_line] = terminal_buffer[next_line];
        }
      }
      // add to last row
      printOnTerminal("Hello, kernel World!\n");
